    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
Lean 4 is an open-source programming language and proof assistant that combines functional programming with formal verification. It allows you to write verified software and mathematical proofs that are mechanically checked for correctness.

Unlike traditional programming languages, Lean 4 has a powerful type system that can express complex properties. If your code type-checks, it's guaranteed to satisfy those properties. This makes it invaluable for critical systems where correctness matters.

Lean 4 represents a major evolution from Lean 3, with significant improvements in performance, metaprogramming capabilities, and language design. It's rapidly becoming the premier tool for formal verification and proof engineering.
`
      },
      {
        id: 'proofs-as-code',
        ...
        kind: 'explanation',
        markdown: `
In Lean 4, proofs are written using tactics—commands that guide the proof assistant to construct a formal proof. Think of tactics as a programming language for proofs, where you build logical arguments step by step.

The structure of a Lean proof follows the \`by ...\` block syntax. Inside, you use tactics to manipulate your proof state until the goal is achieved. This interactive approach makes theorem proving accessible and intuitive.
`
      },
      {
        id: 'first-proof',
        ...
        kind: 'explanation',
        markdown: `
Let's look at a simple example: proving that if a proposition P implies Q, and P implies Q implies R, then P implies R. This is a straightforward chain of logical implications.
...
This proof demonstrates three fundamental tactics: \`intro\` to introduce assumptions, \`apply\` to work backward from the goal, and \`exact\` to provide the exact term needed to close the goal.
`
      },
      {
        id: 'intro-quiz',
        ...
        kind: 'exercise',
        intro: 'Test your understanding of Lean 4 basics with these questions.',
        questions: [
          {
    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
A proof is a logical argument that demonstrates a statement is true. In Lean 4, we write proofs using tactics, which are commands that transform our proof state step by step until we reach the goal.

The simplest form of proof is a direct proof: we start with given assumptions and apply logical rules to reach the desired conclusion.
`
      },
      {
        id: 'intro-and-exact',
        ...
        kind: 'explanation',
        markdown: `
The \`intro\` tactic introduces a hypothesis from an implication in the goal. If your goal is \`P → Q\`, \`intro\` will add \`P\` to your context and change the goal to \`Q\`.

The \`exact\` tactic closes a goal when you have exactly the right term or hypothesis available. It's the "final answer" of your proof.
`
      },
      {
        id: 'intro-exercise',
        ...
        kind: 'exercise',
        intro: 'Test your understanding of the `intro` tactic. What does it do when applied to an implication goal?',
        questions: [
          {
    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
The \`rw\` tactic (rewrite) is one of the most commonly used tactics in Lean. It transforms your goal or hypotheses using equalities.

If you have \`h : a = b\` and your goal contains \`a\`, you can use \`rw [h]\` to replace all occurrences of \`a\` with \`b\`. To rewrite in a hypothesis instead of the goal, use \`rw [h] at hypName\`.
`
      },
      {
        id: 'rw-practice',
        ...
        kind: 'practice',
        intro: 'Practice your rewriting skills with this step-by-step problem.',
        problems: [
          {
            id: 'basic-rewrite',
            title: 'Basic Rewrite',
    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
Mathematical induction is a proof technique for statements about natural numbers. It has two steps: a base case (prove for 0) and an inductive step (prove that if it holds for n, it holds for n+1).

In Lean, use \`induction n with\` to perform induction on variable \`n\`. This will create two subgoals: the base case and the inductive step with an inductive hypothesis available.
`
      },
      {
        ...
        kind: 'practice',
        intro: 'Practice proving a simple property by induction.',
        problems: [
          {
            id: 'add-zero',
            title: 'Add Zero',
    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
Proof by contradiction is a powerful technique where we assume the negation of what we want to prove, then show that this assumption leads to a logical contradiction (False). If assuming ¬P leads to False, then P must be true.

In Lean 4, the \`by_contra\` tactic implements proof by contradiction. If your goal is to prove proposition \`P\`, then \`by_contra h\` introduces a hypothesis \`h : ¬P\` (the negation of P) and changes your goal to proving \`False\`.

This technique is particularly useful when a direct proof seems difficult, but assuming the opposite leads quickly to a contradiction with known facts.
`
      },
      {
        id: 'by-contra-basics',
        ...
        kind: 'explanation',
        markdown: `
The \`by_contra\` tactic follows a clear pattern. After calling \`by_contra h\`, you have \`h : ¬P\` in your context and need to prove \`False\`. To succeed, you must derive a contradiction using \`h\` together with your other hypotheses.
...
In this example, we assume \`P\` is false (introducing \`np : ¬P\`), then use the double negation \`h : ¬¬P\` (which is a function \`¬P → False\`) with \`np\` to derive \`False\` directly.
`
      },
      {
        id: 'by-cases-tactic',
        ...
        kind: 'explanation',
        markdown: `
The \`by_cases\` tactic is a related tool that splits your proof into two cases. For a proposition \`P\`, \`by_cases h : P\` creates two subgoals: one where \`P\` is true, and another where \`¬P\` is true.

This exhaustive case split is useful when you need to consider all possibilities. Combined with proof by contradiction in one branch, it can establish your result holds in all cases.
...
`
      },
      {
        ...
        kind: 'explanation',
        markdown: `
When working with negations and complex logical expressions, the \`push_neg\` tactic is invaluable. It systematically pushes negations inward using logical equivalences, transforming expressions into more manageable forms.

Key transformations performed by \`push_neg\` include: ¬∀ becomes ∃¬ (not for all = there exists not), ¬∃ becomes ∀¬ (not exists = for all not), ¬(A ∧ B) becomes A → ¬B, and ¬(A ∨ B) becomes ¬A ∧ ¬B.

Using \`push_neg at h\` applies these transformations to a hypothesis \`h\` in your context, making it easier to work with in your proof.
`
      },
      {
        ...
        kind: 'exercise',
        intro: 'Test your understanding of proof by contradiction techniques.',
        questions: [
          {
      ...
      },
      {
        ...
        kind: 'practice',
        intro: 'Practice your proof by contradiction skills with this step-by-step problem.',
        problems: [
          {
    ...
    sections: [
      {
        ...
        kind: 'explanation',
        markdown: `
The intersection of AI and formal theorem proving is one of the most exciting frontiers in mathematics and computer science. Lean 4 has become a leading platform for developing and testing AI-powered theorem provers.

Recent advances in 2025-2026 have demonstrated that AI systems can now assist with formal proofs at unprecedented levels. From auto-completing proof steps to suggesting entire proof strategies, AI is transforming how we approach formal verification.

Major developments include benchmarks like SorryDB for evaluating AI provers, systems like Aristotle and Ax-Prover that use multi-agent approaches, and autonomous provers like AxiomProver that have solved previously unsolved problems.
`
      },
      {
        ...
        kind: 'explanation',
        markdown: `
AI-assisted proving in Lean 4 takes several forms. The most common is proof step suggestion, where the AI analyzes your current proof state and suggests relevant tactics or lemmas that might advance your proof.

More advanced systems can generate entire proofs automatically. These systems use large language models trained on vast corpora of formal mathematics, combined with verification loops that ensure each suggested step is correct before proceeding.

The integration of AI with Lean 4's tactic framework enables a new paradigm: interactive theorem proving where human intuition guides the overall strategy while AI handles routine steps and suggests creative approaches when stuck.
`
      },
      {
        ...
        kind: 'explanation',
        markdown: `
LeanDojo is a comprehensive platform for AI-assisted theorem proving in Lean 4. It provides tools for training, evaluating, and deploying AI models that can interact with Lean's proof environment.

The platform offers datasets of formal proofs, evaluation benchmarks, and interfaces that allow AI models to receive feedback from Lean's type checker. This closed-loop system ensures that AI suggestions are always verified before acceptance.

Researchers use LeanDojo to develop and compare different approaches to AI theorem proving, driving rapid advancement in the field. The platform has become the standard for evaluating AI provers on real formal mathematics.
`
      },
      {
        ...
        kind: 'explanation',
        markdown: `
The future of AI in theorem proving points toward more sophisticated collaboration between human mathematicians and AI systems. Emerging approaches include multi-agent systems where specialized AI components handle different aspects of proving.

Another frontier is auto-formalization—using AI to translate informal mathematical text into formal Lean code. This could dramatically accelerate the formalization of research-level mathematics.

As these technologies mature, we may see AI systems that can not only prove theorems but also discover new mathematical conjectures and contribute to the advancement of mathematics itself.
`
      },
      {
        id: 'ai-quiz',
        title: 'Check Your Understanding',
        kind: 'exercise',
        intro: 'Test your understanding of AI-assisted theorem proving.',
        questions: [
          {
